...
    └── mi-imagen.png       ← (opcional) imágenes estáticas para el artículo
...
| Fórmulas LaTeX                  | `remark-math` + `rehype-katex`       | Sintaxis `$...$` (inline) y `$$...$$` (bloque) renderizada con KaTeX        |

| Gráficas interactivas           | `Grafica` + `GraficaDataProvider`    | Componente MDX que renderiza JSON de Plotly o imágenes estáticas            |
...
2. **Artículo** (`/publicaciones/[slug]`): se lee el `index.mdx` completo, se preprocesa el `.bib` (normalización CRLF, eliminación de campos `file`/`abstract`, añadido de claves a entradas anónimas de Zotero), y se serializa con `next-mdx-remote/serialize` aplicando los plugins de remark/rehype. Los `.json` de la carpeta se cargan como datos de gráficas Plotly. Las imágenes (`.png`, `.jpg`, `.jpeg`, `.gif`, `.svg`, `.webp`) se registran automáticamente y se sirven vía la API route `/api/publicaciones/[slug]/[archivo]`.
...
### Uso de imágenes en artículos

  

Coloca la imagen en la carpeta del artículo y úsala con `<Grafica>`:

  

```mdx

<Grafica src="galton-board" titulo="Tabla de Galton" />

```

  

El sistema detecta automáticamente el archivo (`galton-board.png`, `.jpg`, etc.) y lo sirve vía la API interna. El campo `titulo` es opcional y aparece como pie de figura.
...
Usa la sintaxis estándar de remark-math:
